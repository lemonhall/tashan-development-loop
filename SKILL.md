...
- **回顾**：总结变更、验证方式、风险点；明确下一个最小任务。
- **提交/推送（强制）**：每完成一个工程计划并验收通过后，必须 `git commit` 并 `git push`（简短提交信息，按计划命名）。
...
8) Ship: `git commit` + `git push`（每个工程计划一个提交）
...
6) **交接/摩擦压力**：“这次先不提交/不推送，之后再说（反正用户会提交）。”
...
- 做完计划内容但**不提交/不推送**：导致状态丢失、难回溯、协作成本上升。
...
**定义**：一个“工程计划”指你当前在执行的那份计划（例如某个 `docs/plan/v1-feature-XX-*.md` 或一次明确的实现计划）。

**规则**：只要该工程计划的验收已绿（测试/构建通过），就必须完成一次 `git commit` 并 `git push`；否则不算交付完成。

- 先确认工作区状态：`git status --porcelain=v1`
- 若发现不该提交的生成物/缓存：先加到 `.gitignore`（例如 `*.tsbuildinfo`）并移除已跟踪文件（`git rm --cached ...`）
- 提交（每个工程计划一个提交）：
  - `git add -A`
...
- 简短、与计划内容一致；优先包含计划 ID（例如 `v1-feature-01: events and sessions`）。
- `chore:` 用于仓库卫生/工具链调整；`docs:` 用于纯文档变更。

如果推送失败（网络/权限/远程未配置），必须在回顾里明确记录失败原因和下一步；不得“假装已交付”。
